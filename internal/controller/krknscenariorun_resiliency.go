...
//
// Always persists per-cluster score entries with a Status of "calculated" or
// "error" so that API consumers can see exactly which clusters succeeded and
// which failed. Returns an error when any cluster has status "error" so the
// reconciler requeues for retry.
func (r *KrknScenarioRunReconciler) calculateResiliencyScores(
	...
	if hasErrors {
		logger.Info("resiliency scores incomplete, will retry",
			"scenarioRun", scenarioRun.Name,
			"totalScores", len(clusterScores))
		return fmt.Errorf("some cluster resiliency scores could not be calculated")
	}
