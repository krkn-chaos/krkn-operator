...
func TestDeleteTargetRequest_Success(t *testing.T) {
	scheme := runtime.NewScheme()
	krknv1alpha1.AddToScheme(scheme)
	corev1.AddToScheme(scheme)
	...
	targetRequest := &krknv1alpha1.KrknTargetRequest{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Labels: map[string]string{
				"krkn.krkn-chaos.dev/uuid": "test-uuid-123",
			},
		},
		Spec: krknv1alpha1.KrknTargetRequestSpec{
			UUID: "test-uuid-123",
	...
	err := handler.deleteTargetRequest(ctx, "test-uuid-123")

	if err != nil {
		t.Errorf("Expected no error, got %v", err)
	}

	// Verify the target request was deleted
	...
	err = fakeClient.List(ctx, targetRequests, listOpts...)
	if err != nil {
		t.Fatalf("Failed to list target requests: %v", err)
	}

	if len(targetRequests.Items) != 0 {
		t.Errorf("Expected 0 target requests after deletion, got %d", len(targetRequests.Items))
	}
}

func TestDeleteTargetRequest_NotFound(t *testing.T) {
	scheme := runtime.NewScheme()
	krknv1alpha1.AddToScheme(scheme)
	corev1.AddToScheme(scheme)
	...
	ctx := context.Background()
	err := handler.deleteTargetRequest(ctx, "nonexistent-uuid")

	// Should not error when target request is not found
	if err != nil {
		t.Errorf("Expected no error when target request not found, got %v", err)
	}
}

func TestDeleteTargetRequest_MultipleRequests(t *testing.T) {
	scheme := runtime.NewScheme()
	krknv1alpha1.AddToScheme(scheme)
	corev1.AddToScheme(scheme)

	// Create multiple target requests with the same UUID (edge case)
	targetRequest1 := &krknv1alpha1.KrknTargetRequest{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-request-1",
			Namespace: "default",
			Labels: map[string]string{
				"krkn.krkn-chaos.dev/uuid": "duplicate-uuid",
			},
		},
		Spec: krknv1alpha1.KrknTargetRequestSpec{
			UUID: "duplicate-uuid",
	...
	targetRequest2 := &krknv1alpha1.KrknTargetRequest{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-request-2",
			Namespace: "default",
			Labels: map[string]string{
	...
	fakeClient := fakeclient.NewClientBuilder().WithScheme(scheme).
		WithObjects(targetRequest1, targetRequest2).Build()
	fakeClientset := fake.NewSimpleClientset()
	handler := NewHandler(fakeClient, fakeClientset, "default", "localhost:50051")

	ctx := context.Background()
	err := handler.deleteTargetRequest(ctx, "duplicate-uuid")

	if err != nil {
		t.Errorf("Expected no error, got %v", err)
	}

	// Verify both target requests were deleted
	targetRequests := &krknv1alpha1.KrknTargetRequestList{}
	listOpts := []client.ListOption{
		client.InNamespace("default"),
		client.MatchingLabels{
